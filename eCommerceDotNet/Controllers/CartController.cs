...
namespace eCommerceDotNet.Controllers
{
    ...
    [Route("[action]")]
    [ApiController]
    // cart api controller to handle the calls
    public class CartController : ControllerBase
    {
        ...
        public async Task<CartResponse> AddtoCart([FromBody] AddCartReq Req)
        {
            ...
            // checking whether the item is already exist or not
            if (cartData.Count == 0)
            {
                var cart = new Cart();
        ...
        // updating the quantity of a perticular cart item
        [HttpPost]
        ...
        // creating the response for all the cart api calls
